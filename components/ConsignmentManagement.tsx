        ...
        <div className="p-6 bg-gray-800 rounded-lg shadow-md">
            ...
            <div className="space-y-4">
                {consignments.map(con => (
                    <div key={con.id} className="bg-gray-900/50 p-4 rounded-lg">
                        <div className="flex justify-between items-center mb-2">
                            <div>
                                <p className="font-bold text-lg">{supplierMap.get(con.supplierId)} to {branchMap.get(con.branchId)}</p>
                                <p className="text-sm text-gray-400">Received on: {con.receivedDate.toLocaleDateString()}</p>
                            </div>
                             <span className={`px-2 py-1 text-xs font-semibold rounded-full ${con.status === 'ACTIVE' ? 'bg-green-500/20 text-green-300' : 'bg-gray-500/20 text-gray-300'}`}>{con.status}</span>
                        </div>
                        <table className="w-full text-sm text-left mt-2">
                             <thead className="border-b border-gray-700"><tr><th className="p-2">Product</th><th className="p-2 text-right">Received</th><th className="p-2 text-right">Sold</th><th className="p-2 text-right">Remaining</th><th className="p-2 text-right">Cost</th><th className="p-2 text-right">Value Sold</th></tr></thead>
                             <tbody>
                                {con.items.map(item => (
                                    <tr key={item.variantId} className="border-b border-gray-700/50">
                    ...
                    <div className="bg-gray-800 rounded-lg p-6 w-full max-w-3xl max-h-[90vh] flex flex-col">
                        ...
                        <div className="flex-grow overflow-y-auto pr-2 space-y-4">
                            <div className="grid grid-cols-2 gap-4">
                                <div><label className="text-sm">Supplier</label><select onChange={e => setFormState({...formState, supplierId: e.target.value})} className="w-full bg-gray-700 p-2 rounded-md mt-1">{suppliers.map(s=><option key={s.id} value={s.id}>{s.name}</option>)}</select></div>
                                <div><label className="text-sm">Destination Branch</label><select onChange={e => setFormState({...formState, branchId: e.target.value})} className="w-full bg-gray-700 p-2 rounded-md mt-1">{branches.map(b=><option key={b.id} value={b.id}>{b.name}</option>)}</select></div>
                            </div>
                             ...
                             <div className="space-y-2">
                                {formState.items.map((item, index) => (
                                    <div key={index} className="flex gap-2 items-center bg-gray-900/50 p-2 rounded-md">
                                        ...
                                        <input type="number" placeholder="Qty Received" value={item.quantityReceived} onChange={e => handleItemChange(index, 'quantityReceived', e.target.value)} className="w-1/4 bg-gray-700 p-2 rounded-md text-sm" />
                                        <input type="number" placeholder="Cost Price" value={item.costPrice} onChange={e => handleItemChange(index, 'costPrice', e.target.value)} className="w-1/4 bg-gray-700 p-2 rounded-md text-sm" />
                        ...
                        <div className="flex justify-end gap-3 mt-6 pt-4 border-t border-gray-700">
                            <button onClick={() => setModalOpen(false)} className="px-4 py-2 rounded-md bg-gray-600 hover:bg-gray-500">Cancel</button>
                            <button onClick={handleSubmit} className="px-4 py-2 rounded-md bg-indigo-600 hover:bg-indigo-500">Add Intake</button>
                        </div>
                    </div>
                </div>
            )}
