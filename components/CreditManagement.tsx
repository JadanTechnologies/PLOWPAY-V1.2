            ...
            {isPaymentModalOpen && selectedCustomer && (
                <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4">
                    <div className="bg-slate-800 rounded-lg p-6 w-full max-w-md">
                        ...
                        <p className="text-slate-400">Current Balance: <span className="font-bold text-red-400">{formatCurrency(selectedCustomer.creditBalance)}</span></p>
                        <div className="mt-4">
                            <label className="text-sm">Payment Amount</label>
                            <input type="number" value={paymentAmount} onChange={e => setPaymentAmount(e.target.value)} max={selectedCustomer.creditBalance} className="w-full bg-slate-700 p-2 rounded-md mt-1" />
                        </div>
                        <div className="flex justify-end gap-3 mt-6">
                            <button onClick={() => setPaymentModalOpen(false)} className="px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-500">Cancel</button>
                            <button onClick={handleRecordPayment} className="px-4 py-2 rounded-md bg-cyan-600 hover:bg-cyan-500">Confirm Payment</button>
                        </div>
                    </div>
                </div>
            )}
