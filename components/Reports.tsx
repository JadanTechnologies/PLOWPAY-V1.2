...
const CustomerCreditReport: React.FC<{ customers: Customer[], formatCurrency: (val: number) => string }> = ({ customers, formatCurrency }) => {
    // FIX: Explicitly cast `creditBalance` to a number to resolve a type error where it was being inferred as 'unknown'.
    const creditCustomers = useMemo(() => customers.filter(c => Number(c.creditBalance) > 0), [customers]);
    // FIX: Explicitly cast `creditBalance` to a number to resolve the type error.
...
const Reports: React.FC = () => {
  const { sales, products, branches, staff, customers, purchaseOrders, consignments, suppliers, currentTenant, isLoading, setNotification, currentStaffUser, staffRoles } = useAppContext();
 ...
 const reportBranchName = useMemo(() => {
    ...
    return branches.find(b => b.id === branchFilter)?.name || 'N/A';
}, [branchFilter, branches]);

const generatedBy = useMemo(() => {
    return currentStaffUser?.name || currentTenant?.ownerName || 'N/A';
}, [currentStaffUser, currentTenant]);
  ...
  return (
    <div id="report-printable-area">
        <div className="print-header" style={{ display: 'none' }}>
            <h1 className="business-name">{currentTenant?.businessName}</h1>
            <h2 className="report-title">{reportTitles[activeReport]}</h2>
            <div className="report-details">
                <span><strong>Branch:</strong> {reportBranchName}</span>
                <span><strong>Period:</strong> {dateRange.start.toLocaleDateString()} to {dateRange.end.toLocaleDateString()}</span>
