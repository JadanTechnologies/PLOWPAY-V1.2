...
const PointOfSale: React.FC = () => {
  ...
  const currentBranchId = useMemo(() => currentStaffUser?.branchId || branches[0]?.id, [currentStaffUser, branches]);
  ...
  const filteredProducts = useMemo(() => {
    return products.filter(product => {
      ...
      if (currentStaffUser) {
          ...
          return matchesCategory && matchesSearch && matchesFavorites && isVisibleInBranch;
      }

      return matchesCategory && matchesSearch && matchesFavorites;
    });
  }, [products, searchTerm, selectedCategoryId, showFavorites, currentBranchId, currentStaffUser]);
