            ...
            <div id="invoice-modal" className="bg-slate-800 rounded-lg shadow-xl w-full max-w-md max-h-[90vh] flex flex-col border border-slate-700">
                ...
                <div id="invoice-content" className="p-6 overflow-y-auto">
                    ...
                    <table className="w-full text-sm mb-4">
                        <thead>
                            <tr className="border-b-2 border-dashed border-slate-600"><th className="text-left py-1">Item</th><th className="text-center py-1">Qty</th><th className="text-right py-1">Price</th><th className="text-right py-1">Total</th></tr>
                        </thead>
                        <tbody>
                            {sale.items.map(item => (
                                <tr key={item.variantId} className="border-b border-dashed border-slate-700">
                                    <td className="py-1">
                                        <div>{item.name}</div>
                    ...
                    <div className="space-y-1 text-sm mb-4">
                        ...
                        <div className="flex justify-between"><span className="text-slate-400">Tax:</span><span>{formatCurrency(tax)}</span></div>
                         {sale.discount && sale.discount > 0 && <div className="flex justify-between"><span className="text-slate-400">Discount:</span><span className="text-red-400">-{formatCurrency(sale.discount)}</span></div>}
                    ...
                    <p className="text-center text-xs mt-6 text-slate-400">Thank you for your business!</p>
                </div>
