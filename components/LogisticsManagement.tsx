...
const LogisticsManagement: React.FC = () => {
    const { 
      ...
      updateTruckVitals, updateTenantLogisticsConfig, setNotification
    } = useAppContext();
    ...
    const handleSellShipment = async () => {
        if (selectedShipment && sellShipmentForm.name.trim()) {
            const result = await sellShipment(selectedShipment.id, sellShipmentForm);
    ...
    const renderContent = () => {
        switch (activeTab) {
                    ...
                    <div className="space-y-6">
                        ...
                        <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
                            <div className="p-4 bg-slate-800 rounded-lg shadow-md">
                                ...
                                <button onClick={() => setMapModalOpen(true)} className="w-full bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-3 px-4 rounded-md flex items-center justify-center mb-4 transition-transform hover:scale-105">
                                    <Icon name="map-pin" className="w-6 h-6 mr-3"/>View Live Fleet Map
                                </button>
                                <div className="bg-slate-900/50 rounded-lg h-64 flex flex-col p-2 overflow-y-auto">
                                    {trucks.map(truck => (
                                        <div key={truck.id} className="flex items-center p-2 rounded-md hover:bg-slate-700/50">
                                            <Icon name="truck" className={`w-6 h-6 mr-3 ${truck.status === 'IN_TRANSIT' ? 'text-cyan-400' : 'text-slate-500'}`} />
                                            <div>
                                                <p className="font-semibold">{truck.licensePlate} - {truck.driverName}</p>
                                                <p className="text-sm text-slate-400">{truck.currentLocation.address}</p>
                                            </div>
                                        ...
                                        {shipments.filter(s => s.status === 'IN_TRANSIT' || s.status === 'PENDING').map(shipment => (
                                            <tr key={shipment.id} className="border-b border-slate-700">
                                                <td className="p-2">
                                                    ...
                                                    <p className="text-sm text-slate-400">{shipment.origin} to {branchMap.get(shipment.destination)}</p>
                                                </td>
                 ...
                 return (
                     <div className="p-6 bg-slate-800 rounded-lg shadow-md">
                         <div className="flex justify-between items-center mb-6">
                            ...
                            <button onClick={() => handleOpenModal('TRUCK')} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded-md flex items-center">
                                <Icon name="plus" className="w-5 h-5 mr-2" /> Add Truck
                            </button>
                         </div>
                         <div className="overflow-x-auto">
                           <table className="w-full text-left">
                               <thead className="border-b border-slate-700 text-xs uppercase">
                                   <tr>
                               ...
                               <tbody>
                                {trucks.map(truck => (
                                    <tr key={truck.id} className="border-b border-slate-700 hover:bg-slate-700/50 text-sm">
                                        ...
                                        <td className="p-3">{truck.currentLocation.address}</td><td className="p-3 text-slate-400">{truck.lastUpdate.toLocaleString()}</td><td className="p-3 text-center">{getStatusBadge(truck.status)}</td>
                               ...
                               <tbody>
                                {shipments.map(shipment => (
                                    <tr key={shipment.id} className="border-b border-slate-700 hover:bg-slate-700/50">
                                        ...
                                        <td className="p-3 text-sm text-slate-400">{shipment.estimatedDelivery.toLocaleDateString()}</td>
            ...
            case 'settings':
                 ...
                 return (
                     <div className="p-6 bg-slate-800 rounded-lg shadow-md max-w-2xl mx-auto">
                         <h2 className="text-2xl font-bold text-white mb-6">Tracker Provider Settings</h2>
                         <div className="space-y-6">
                            <div>
                                <label className="block text-sm font-medium text-slate-400">Active Tracker Provider</label>
                                <select
                                    value={activeTrackerProviderId}
                                    onChange={e => updateTenantLogisticsConfig({ activeTrackerProviderId: e.target.value })}
                                    className="mt-1 block w-full bg-slate-700 border border-slate-600 rounded-md py-2 px-3 text-white focus:ring-cyan-500 focus:border-cyan-500"
                                >
                                ...
                                <div className="bg-slate-900/50 p-4 rounded-lg border border-slate-700">
                                    ...
                                    <div className="space-y-4">
                                        <div>
                                         ...
                                         <div>
                                            <label className="block text-sm font-medium text-slate-400">API Endpoint URL</label>
                                            <input 
                                                type="text" 
                         ...
                         <div className="text-right mt-6">
                            <button onClick={handleSaveTrackerSettings} className="bg-cyan-600 hover:bg-cyan-500 text-white font-bold py-2 px-4 rounded-md">Save Settings</button>
                         </div>
                     </div>
                );
        }
            ...
            {modal === 'TRUCK' && (
                <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4">
                    <div className="bg-slate-800 rounded-lg p-6 w-full max-w-md">
                        ...
                        <div className="space-y-4">
                            <div><label className="text-sm">License Plate</label><input type="text" onChange={e => setTruckForm({...truckForm, licensePlate: e.target.value})} className="w-full bg-slate-700 p-2 rounded-md mt-1"/></div>
                            <div><label className="text-sm">Driver Name</label><input type="text" onChange={e => setTruckForm({...truckForm, driverName: e.target.value})} className="w-full bg-slate-700 p-2 rounded-md mt-1"/></div>
                            <div><label className="text-sm">Max Load (kg)</label><input type="number" value={truckForm.maxLoad} onChange={e => setTruckForm({...truckForm, maxLoad: parseInt(e.target.value, 10) || 0})} className="w-full bg-slate-700 p-2 rounded-md mt-1"/></div>
                            <div><label className="text-sm">Status</label><select onChange={e => setTruckForm({...truckForm, status: e.target.value as Truck['status']})} className="w-full bg-slate-700 p-2 rounded-md mt-1"><option>IDLE</option><option>IN_TRANSIT</option><option>MAINTENANCE</option></select></div>
                        </div>
                        <div className="flex justify-end gap-3 mt-6"><button onClick={handleCloseModal} className="px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-500">Cancel</button><button onClick={handleAddTruck} className="px-4 py-2 rounded-md bg-cyan-600 hover:bg-cyan-500">Save</button></div>
                    </div>
                </div>
            )}
                 ...
                 <div className="fixed inset-0 bg-black bg-opacity-70 z-50 flex justify-center items-center p-4">
                    <div className="bg-slate-800 rounded-lg p-6 w-full max-w-md">
                        <h3 className="text-xl font-bold mb-4">Sell Cargo of {selectedShipment.shipmentCode}</h3>
                         <p className="text-sm text-slate-400 mb-4">This will create a direct sale for all items in the shipment to a single customer.</p>
                        <div className="space-y-4">
                            <div><label className="text-sm">Customer Name</label><input type="text" value={sellShipmentForm.name} onChange={e => setSellShipmentForm({...sellShipmentForm, name: e.target.value})} className="w-full bg-slate-700 p-2 rounded-md mt-1"/></div>
                            <div><label className="text-sm">Customer Phone (Optional)</label><input type="tel" value={sellShipmentForm.phone} onChange={e => setSellShipmentForm({...sellShipmentForm, phone: e.target.value})} className="w-full bg-slate-700 p-2 rounded-md mt-1"/></div>
                        </div>
                        <div className="flex justify-end gap-3 mt-6"><button onClick={handleCloseModal} className="px-4 py-2 rounded-md bg-slate-600 hover:bg-slate-500">Cancel</button><button onClick={handleSellShipment} className="px-4 py-2 rounded-md bg-purple-600 hover:bg-purple-500">Confirm Sale</button></div>
                    </div>
                </div>
            )}
            {isMapModalOpen && (
                <div className="fixed inset-0 bg-black/90 z-50 flex flex-col p-4">
                    <div className="flex justify-between items-center mb-4 flex-shrink-0">
                        ...
                        <button onClick={() => setMapModalOpen(false)} className="bg-slate-700 hover:bg-slate-600 text-white rounded-full p-2">
                            <Icon name="x-mark" className="w-6 h-6" />
                        </button>
                    </div>
                    <div className="relative flex-grow bg-slate-800 rounded-lg overflow-hidden">
