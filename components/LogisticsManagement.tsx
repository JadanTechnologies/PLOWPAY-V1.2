...
import { Truck, Shipment, TrackerProvider } from '../types';
...
const LogisticsManagement: React.FC = () => {
    const { trucks, shipments, trackerProviders, addTruck, updateTruck, addShipment, updateShipmentStatus, updateTrackerProvider } = useAppContext();
    const [activeTab, setActiveTab] = useState<'dashboard' | 'trucks' | 'shipments' | 'settings'>('dashboard');
                                        ...
                                        {shipments.filter(s => s.status === 'IN_TRANSIT' || s.status === 'PENDING').map(shipment => (
                                            <tr key={shipment.id} className="border-b border-gray-700">
                                                <td className="p-2">
                                                    ...
                                                    <p className="text-sm text-gray-400">{shipment.origin} to {shipment.destination}</p>
                                                </td>
            ...
            case 'trucks':
                return (
                     <div className="p-6 bg-gray-800 rounded-lg shadow-md">
                         <h2 className="text-2xl font-bold text-white mb-6">Manage Trucks</h2>
                               ...
                               <tbody>
                                {trucks.map(truck => (
                                    <tr key={truck.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                                        ...
                                        <td className="p-3">{truck.currentLocation.address}</td>
                                        <td className="p-3 text-sm text-gray-400">{truck.lastUpdate.toLocaleString()}</td>
                                        <td className="p-3 text-center">{getStatusBadge(truck.status)}</td>
                                    </tr>
                                ))}
                               </tbody>
                           </table>
             ...
             case 'shipments':
                 return (
                     <div className="p-6 bg-gray-800 rounded-lg shadow-md">
                         <h2 className="text-2xl font-bold text-white mb-6">Manage Shipments</h2>
                         <div className="overflow-x-auto">
                           <table className="w-full text-left">
                               <thead className="border-b border-gray-700">
                                   <tr>
                               ...
                               <tbody>
                                {shipments.map(shipment => (
                                    <tr key={shipment.id} className="border-b border-gray-700 hover:bg-gray-700/50">
                                        ...
                                        <td className="p-3">{shipment.origin} <Icon name="chevronRight" className="w-4 h-4 inline-block"/> {shipment.destination}</td>
                                        <td className="p-3">{trucks.find(t => t.id === shipment.truckId)?.licensePlate || 'N/A'}</td>
